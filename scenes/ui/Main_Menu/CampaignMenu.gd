...
func _scan_folder_for_levels(folder_path: String) -> Array:
	...
	# In exported builds, use LevelManifest (DirAccess won't work with packed resources)
	...
	# In editor, use DirAccess (faster and more reliable)
	...
	if dir != null:
		...
		while file_name != "":
			...
			if not file_name.begins_with(".") and not dir.current_is_dir() and file_name.ends_with(".tscn"):
	...
	# Fallback if DirAccess fails in editor (shouldn't happen)
	var folder_name = folder_path.trim_suffix("/").get_file()
